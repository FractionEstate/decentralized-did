...
def _extract_metadata_payload(metadata: Dict[str, object]) -> Dict[str, object]:
    ...
    cip30_entries = metadata.get("metadata") if "metadata" in metadata else None
...
def cmd_generate(args: argparse.Namespace) -> None:
    ...
    master_digest, helper_map, _ = _compute_enrollment(input_payload["fingers"])
...
def cmd_demo_kit(args: argparse.Namespace) -> None:
    ...
    _write_json(wallet_inline_path, inline_bundle.metadata_for_format("wallet"))
    ...
    _write_json(wallet_external_path, external_bundle.metadata_for_format("wallet"))
    files_created.append(wallet_external_path)

    cip30_external_path = output_dir / "metadata_cip30_external.json"
    _write_json(cip30_external_path, external_bundle.metadata_for_format("cip30"))
    ...
    helper_ts_json = json.dumps(helper_map, indent=2)

    entry_blocks: List[str] = []
    for label, payload in cip30_inline["metadata"]:
        payload_json = json.dumps(payload, indent=2)
        indented_payload = textwrap.indent(payload_json, "    ")
        entry_blocks.append(f"  [BigInt({label}),\n{indented_payload}\n  ]")

    entries_section = ",\n".join(entry_blocks)
    helper_ts_formatted = textwrap.indent(helper_ts_json, "  ")
    ...
    ts_lines = [
        ...
        "export const cip30MetadataEntries = new Map<bigint, unknown>([",
    ]
    if entries_section:
        ts_lines.append(entries_section)
    ts_lines.extend(
        [
            "]);",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    demo.add_argument("--label", type=int, help="Cardano metadata label override (default 1990)")
    demo.add_argument(
        "--helper-uri",
    ...
    demo.add_argument("--quiet", action="store_true", help="suppress stdout output")
