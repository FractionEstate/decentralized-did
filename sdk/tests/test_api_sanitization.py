...
class TestWhitespaceNormalization:
    ...
    def test_normalize_whitespace_no_collapse(self):
        ...
        result = normalize_whitespace(
            "hello    world", collapse_spaces=False, trim=True)
        assert result == "hello    world"

    def test_normalize_whitespace_no_trim(self):
        """Test whitespace normalization without trimming"""
        result = normalize_whitespace(
            "  hello  ", collapse_spaces=False, trim=False)
        assert result == "  hello  "

    def test_limit_consecutive_spaces(self):
        """Test limiting consecutive spaces"""
        assert limit_consecutive_spaces(
            "a" + " " * 20 + "b", max_spaces=5) == "a" + " " * 5 + "b"
        assert limit_consecutive_spaces(
            "normal  spacing", max_spaces=10) == "normal  spacing"
