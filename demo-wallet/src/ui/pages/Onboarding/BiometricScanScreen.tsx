...
const BiometricScanScreen = ({
  ...
  const [skippedFingers, setSkippedFingers] = useState<Set<string>>(new Set());
  ...
  const rightHandFingers = useMemo(
    () => fingersToScan.filter((f) => f.startsWith("right-")),
    [fingersToScan]
  );
  const leftHandFingers = useMemo(
    () => fingersToScan.filter((f) => f.startsWith("left-")),
    [fingersToScan]
  );
  ...
  const skipFinger = useCallback(() => {
    ...
    setSkippedFingers((prev) => new Set(prev).add(currentFinger));
  ...
  }, [capturedData, currentFinger, currentFingerIndex, fingersToScan.length, onComplete]);
      ...
      <div className="finger-checklist" data-testid="finger-checklist">
        ...
        <div className="hands-container">
          <div className="hand-section">
            <h4 className="hand-section__title">Right Hand</h4>
            {rightHandFingers.map((finger) => {
              const index = fingersToScan.indexOf(finger);
              const status =
                skippedFingers.has(finger)
                  ? "skipped"
              ...
              return (
                ...
                >
                  ...
                  {status === "active" && !scanning && !scanSuccess && (
                    <button
                      className="finger-item__skip"
                      onClick={skipFinger}
                      aria-label={`Skip ${formatFingerName(finger)}`}
                    >
                      Skip
                    </button>
                  )}
          ...
          <div className="hand-section">
            <h4 className="hand-section__title">Left Hand</h4>
