...
class PeerConnectionStorage {
  ...
  async getPeerConnection(id: string): Promise<PeerConnection> {
    ...
    return {
      ...
      biometricMetadata: metadata.biometricMetadata,
    };
  }
  ...
  async updatePeerConnectionMetadata(
    ...
    metadata: Partial<
      Pick<
        PeerConnectionMetadataRecord,
        "name" | "url" | "iconB64" | "selectedAid" | "biometricMetadata"
      >
    >
  ): Promise<void> {
    ...
    if (metadata.biometricMetadata !== undefined)
      identifierMetadataRecord.biometricMetadata = metadata.biometricMetadata;
