...
import type { Cip30MetadataEnvelope } from "./peerConnection.types";
...
describe("IdentityWalletConnect", () => {
  beforeEach(() => {
    identityWalletConnect.setActiveDAppAddress(null);
  });
  ...
  test("should store biometric metadata and emit event", async () => {
    ...
    identityWalletConnect.setActiveDAppAddress(dAppAddress);
    Agent.agent.peerConnectionMetadataStorage.getPeerConnectionMetadata = jest
      .fn()
      .mockResolvedValue({ id: dAppAddress });
    Agent.agent.peerConnectionMetadataStorage.updatePeerConnectionMetadata =
      jest.fn().mockResolvedValue(undefined);
    eventServiceMock.emit = jest.fn();

    await identityWalletConnect.storeBiometricMetadata(metadataEnvelope);
  ...
  test("should throw when storing metadata without connection", async () => {
    ...
    await expect(
      identityWalletConnect.storeBiometricMetadata(metadataEnvelope)
    ).rejects.toThrow("No connected dApp available for biometric metadata");
  });
});
